            ...
            {scrollingData.map((item, index) => (
              <div key={index} className="w-full">
                <div className="relative w-full h-100 rounded-3xl bg-linear-to-r from-[#2D4E86] via-[#2E7D63] to-[#2F95E1] p-6 flex  flex-col items-center justify-center gap-4">
                  ...
                  <div
                    className={`flex flex-col items-end justify-start flex-1 h-full py-2 gap-3  w-full  ${
                      isArabic ? "text-right" : "text-left"
                    }`}
                  >
                    <div
                      className={`bg-white px-5 py-1 rounded-2xl text-[#378BC8] font-bold  ${
                        isArabic ? "text-xl" : "font-roboto text-base  "
                      }`}
                    >
                      <h5>{item.title}</h5>
                    </div>
                    ...
                    >
                      {Array.isArray(item.text) ? (
                        ...
                        <ul
                          className={`list-disc text-white/90 text-sm  h-20 ${
                            isArabic
